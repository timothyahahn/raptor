...
void explore_history()
{
	...
	while(!key[KEY_TAB] && !key[KEY_ESC])
	{
		...
		if(mr || (mb && graphfunction == 5)) //moving the graph
		{
			my = mouse_y;
			mx = mouse_x;
			if(my > (SCRNHEI - 150) && mx > 40 && mx < SCRNWID - 10 && edge != nullptr)
			{	explore_time = threadZero->getUsageHistStartTime() + (mx - 40) * usageWindow / (SCRNWID - 50);	
				paint_link_usage(explore_time);
...
void topo_move()
{
	...
	{
		clear_to_color(edgesbmp,makecol(255,0,255));
		for(unsigned int r = 0; r < threadZero->getNumberOfRouters(); ++r)//TODO: much of this is also in explore_history. Don't copy/paste code.
		{
...
void flash()
{
	flashscreen = load_bitmap("bitmaps/flashscreen.bmp",nullptr); //flashscreen for ALL-GUI
...
{
	int from = -1;//this is copied too. Again, TODO: we should replace with a return index function
	int to = -1;
	int spans = 0;
	int mosx = mouse_x;
	int mosy = mouse_y;
