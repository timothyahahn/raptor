...
void paint_link_usage(int p) {
  ...
  blit(edgesbmp, mainbuf, topoX1, topoY1, topoX1, topoY1, topoX2 - topoX1,
       topoY2 - topoY1);
  masked_blit(routersbmp, mainbuf, topoX1, topoY1, topoX1, topoY1,
              topoX2 - topoX1, topoY2 - topoY1);
  masked_blit(topbuttons, mainbuf, 0, 0, 0, 0, SCRNWID, topbuttons->h);
  if (showcolorkey) {
    blit(colorkey, mainbuf, 0, 0, SCRNWID - colorkey->w - 10,
         graphY1 - 16 - colorkey->h - 10, SCRNWID, SCRNHEI);
  }
  if (threadZero->getPaintSpans()) {
    masked_blit(edgespans, mainbuf, topoX1, topoY1, topoX1, topoY1,
                topoX2 - topoX1, topoY2 - topoY1);
  }
...
//					parameter n. 0 = index, 1 = probfails... the
...
void paint_router_nums() {
  ...
  stretch_blit(topobackground, mainbuf, 0, 0, topobackground->w,
               topobackground->h, bkgrndX, bkgrndY, realTopoWidthPx,
               realTopoHeightPx);
  masked_blit(edgesbmp, mainbuf, topoX1, topoY1, topoX1, topoY1,
              topoX2 - topoX1, topoY2 - topoY1);  // makes it black
  if (threadZero->getPaintSpans()) {
    masked_blit(edgespans, mainbuf, topoX1, topoY1, topoX1, topoY1,
                topoX2 - topoX1, topoY2 - topoY1);
  }
  ...
  if (edge == nullptr) {
    stretch_blit(graphbackground, mainbuf, 0, 0, graphbackground->w,
                 graphbackground->h, graphX1, graphY1, graphX2 - graphX1,
                 graphY2 - graphY1);
  }
  masked_blit(graphbuttons, mainbuf, 0, 0, 0, graphY1 - 16, SCRNWID, SCRNHEI);
  masked_blit(
      graph, mainbuf, graphX1, graphY1 - topbuttons->h, graphX1,
      graphY1 - topbuttons->h, SCRNWID,
      graphY2 - (graphY1 - topbuttons->h));  // size of graph ALLEGRO_BITMAP
                                             // should really be changed
  if (showcolorkey) {
    blit(colorkey, mainbuf, 0, 0, SCRNWID - colorkey->w - 10,
         graphY1 - 16 - colorkey->h - 10, colorkey->w, colorkey->h);
  }
...
void paint_topo_edges(int p) {
  ...
  masked_blit(routersbmp, mainbuf, topoX1, topoY1, topoX1, topoY1,
              topoX2 - topoX1, topoY2 - topoY1);
  /*if(threadZero->getPaintSpans())
  {	masked_blit(edgespans,
  mainbuf,topoX1,topoY1,topoX1,topoY1,topoX2-topoX1,topoY2-topoY1);	}*/
  masked_blit(mainbuf, screen, 0, topbuttons->h, 0, topbuttons->h,
              topoX2 - topoX1, topoY2 - topoY1);
}
...
void topoRefresh() {
  ...
  masked_blit(graphbuttons, mainbuf, 0, 0, 0, graphY1 - 16, SCRNWID, SCRNHEI);
  /*if(showcolorkey)
  {	blit(colorkey,mainbuf,0,0,SCRNWID-colorkey->w -
  10,graphY1-16-colorkey->h -10,colorkey->w,colorkey->h);	}*/
  masked_blit(popup, mainbuf, 0, 0, 0, 0, SCRNWID, SCRNHEI);
  blit(mainbuf, screen, 0, topbuttons->h, 0, topbuttons->h, topoX2 - topoX1,
       topoY2 - topoY1);
}
...
//					some change to the topology viewer,
//usually 					due to user moving or zooming in.
//
///////////////////////////////////////////////////////////////////
void refreshedges() {
...
//					user to click on graph to choose time of
//interest. 					Runs while loop, updating all edges to show usage 					levels as user
//pushes arrows.
//
///////////////////////////////////////////////////////////////////
void explore_history() {
  ...
  clear_to_color(edgesbmp,
                 makecol(255, 0, 255));  // initialize buffers to magic pink
  clear_to_color(routersbmp, makecol(255, 0, 255));
  clear_to_color(edgespans, makecol(255, 0, 255));
  clear_to_color(mainbuf, makecol(255, 0, 255));
  ...
  while (!key[KEY_TAB] && !key[KEY_ESC]) {
    ...
    if (mb) {
      ...
      } else if (my < (graphY1))  // clicked on graph button bar
      {
        if (mx < graphbutton[0]->w) {
          ...
          graphright();
                   ...
                   graphbutton[0]->w + graphbutton[1]->w + graphbutton[2]->w) {
          graphpoint();
        } else if (mx < graphbutton[0]->w + graphbutton[1]->w +
                            2 * graphbutton[2]->w)  // clicked on zoom in
        {
          graphzoomin();
        } else if (mx < graphbutton[0]->w + graphbutton[1]->w +
                            3 * graphbutton[2]->w)  // clicked on zoom out
        {
          graphzoomout();
        } else if (mx < graphbutton[0]->w + graphbutton[1]->w +
                            4 * graphbutton[2]->w)  // clicked on zoom window
        {
          graphzoomwindow();
        } else if (mx < graphbutton[0]->w + graphbutton[1]->w +
                            5 * graphbutton[2]->w) {
      ...
      } else if ((graphfunction == 1) &&
                 my < (SCRNHEI)) {  // if user clicked on graph with pointer, go
                                    // to that time.
        if (mx < 40) {
          if (threadZero->getUsageHistStartTime() > 0) {
            threadZero->setUsageHistStartTime(
                threadZero->getUsageHistStartTime() - 1);
            explore_time = threadZero->getUsageHistStartTime();
          } else {
            mx = 40;
            explore_time = threadZero->getUsageHistStartTime() +
                           (mx - 40) * usageWindow / (SCRNWID - 50);
          }
        } else {
          explore_time = threadZero->getUsageHistStartTime() +
                         (mx - 40) * usageWindow / (SCRNWID - 50);
        ...
        if (explore_time >= threadZero->getUsageHistStartTime() + usageWindow) {
          if (explore_time >= nums) {
            explore_time =
                threadZero->getUsageHistStartTime() + usageWindow - 1;
          } else {
            threadZero->setUsageHistStartTime(
                threadZero->getUsageHistStartTime() + 1);
        ...
        if (edge != nullptr) {
          ...
          edge->paintUsageHistory(graphX1, graphY1, graphX2, graphY2,
                                  explore_time);
        }
      }
    ...
    if (mr || (mb && graphfunction == 5))  // moving the graph
    {
      my = mouse_y;
      mx = mouse_x;
      if (my > (SCRNHEI - 150) && mx > 40 && mx < SCRNWID - 10 &&
          edge != nullptr) {
        explore_time = threadZero->getUsageHistStartTime() +
                       (mx - 40) * usageWindow / (SCRNWID - 50);
        paint_link_usage(explore_time);
        while ((mr || (mb && graphmove)) && mx > 40 && mx < SCRNWID - 10) {
          edge->paintUsageHistory(graphX1, graphY1, graphX2, graphY2, -1);
          mx = mouse_x;
          timeMov = explore_time - (threadZero->getUsageHistStartTime() +
                                    (mx - 40) * usageWindow / (SCRNWID - 50));
          if (threadZero->getUsageHistStartTime() <= 0 && timeMov < 0) {
            timeMov = 0;
            threadZero->setUsageHistStartTime(0);
          } else if (threadZero->getUsageHistStartTime() >=
                         (threadZero->getRouterAt(0)
                              ->getEdgeByIndex(0)
                              ->getUsageNums() -
                          usageWindow) &&
                     timeMov > 0) {
            timeMov = 0;
            threadZero->setUsageHistStartTime(
                threadZero->getRouterAt(0)->getEdgeByIndex(0)->getUsageNums() -
                usageWindow);
          } else {
            threadZero->setUsageHistStartTime(
                threadZero->getUsageHistStartTime() + timeMov);
          }
          mr = (mouse_b & 2);
          mb = (mouse_b & 1);
          edge->paintUsageHistory(graphX1, graphY1, graphX2, graphY2, -1);
...
void show_spans() {
  ...
  paint_link_usage(explore_time);
  if (threadZero->getPaintSpans())
    blit(topbuttonpress[1], topbuttons, 0, 0, topbuttonpress[0]->w, 0, SCRNWID,
         SCRNHEI);
  else
    blit(topbutton[1], topbuttons, 0, 0, topbuttonpress[0]->w, 0, SCRNWID,
         SCRNHEI);
  paint_router_nums();
}
...
//
///////////////////////////////////////////////////////////////////
void show_workstations() {  // showindexes (1), showwks (2), showfailurefrom
                            // (3), showfailureto (4)
  int overx = 0;
  for (int i = 0; i < 5; i++) {
    overx += topbutton[i]->w;
...
//
///////////////////////////////////////////////////////////////////
void show_failures_from() {  // showindexes (1), showwks (2), showfailurefrom
                             // (3), showfailureto (4)
  int overx = 0;
  for (int i = 0; i < 6; i++) {
    overx += topbutton[i]->w;
...
//
///////////////////////////////////////////////////////////////////
void show_failures_to() {  // showindexes (1), showwks (2), showfailurefrom (3),
                           // showfailureto (4)
  int overx = 0;
  for (int i = 0; i < 7; i++) {
    overx += topbutton[i]->w;
...
void topo_move() {
  ...
  while (mouse_b & 1) {
    ...
    {
      threadZero->getRouterAt(r)->moveXYPixels(
          diffx, diffy);  // initially translate below top button bar
    }
...
void graphleft() {
  ...
  blit(graphbuttonpress[0], graphbuttons, 0, 0, overx, 0, graphbuttons->w,
       graphbuttons->h);
  blit(graphbuttons, screen, 0, 0, 0, graphY1 - graphbutton[0]->h, SCRNWID,
       SCRNHEI);
  ...
  blit(graphbutton[0], graphbuttons, 0, 0, overx, 0, graphbuttons->w,
       graphbuttons->h);
  paint_router_nums();
}
...
void graphright() {
  ...
  blit(graphbuttonpress[1], graphbuttons, 0, 0, overx, 0, graphbuttons->w,
       graphbuttons->h);
  blit(graphbuttons, screen, 0, 0, 0, graphY1 - graphbutton[0]->h, SCRNWID,
       SCRNHEI);
  ...
  blit(graphbutton[1], graphbuttons, 0, 0, overx, 0, graphbuttons->w,
       graphbuttons->h);
  paint_router_nums();
}
...
//					others if necessary. Paints over
//buttons.
//
///////////////////////////////////////////////////////////////////
void graphpoint() {
...
void graphzoomin() {
  ...
  while (key[KEY_UP] || (mouse_b & 1)) {
    if (usageWindow > 50) {
      aPercent = double(explore_time - threadZero->getUsageHistStartTime()) /
                 double(usageWindow);
      startTime = ceil(threadZero->getUsageHistStartTime() +
                       aPercent * shrinkConstant * usageWindow);
    ...
    if (mouse_y > graphY1)  // if clicked in graph area
    {
      ...
      if (abs(choseB - choseA) > 15) {
        ...
        choseA = threadZero->getUsageHistStartTime() +
                 (choseA - 40) * usageWindow / (SCRNWID - 50);
        choseB = threadZero->getUsageHistStartTime() +
                 (choseB - 40) * usageWindow / (SCRNWID - 50);
...
//					its history graph. returns time if
//desired.
//
///////////////////////////////////////////////////////////////////
bool userDisplayEdge()  // return true if it should go to explore edge next
{
  ...
  for (size_t r = 0; r < threadZero->getNumberOfRouters(); ++r) {
    ...
    clickdist = sqrt((dmx - rx) * (dmx - rx) +
                     (dmy - ry) * (dmy - ry));  // distance formula
  ...
  if (mb && source > -1) {  // if we've clicked again and we've already selected
    ...
    for (size_t r2 = 0; r2 < threadZero->getNumberOfRouters(); ++r2) {
      ...
      clickdist = sqrt((dmx - rx) * (dmx - rx) +
                       (dmy - ry) * (dmy - ry));  // distance formula
      if (clickdist < threadZero->getRouterAt(r2)
                          ->getRadius()) {  // this should eventually be a
                                            // global constant I think.
        if (threadZero->getRouterAt(source)->isAdjacentTo(r2) != -1) {
          ...
          // clear_to_color(graph,makecol(255,0,255)); //taken out to fix graph
          // drawing bug
          ...
          paint_router_nums();
          return false;
          // paint_router_nums(); //this is all it was before, after
          // clear_to_color()
        }
      }
    }
...
//
///////////////////////////////////////////////////////////////////
void close_button_handler() { threadZero->setTerminate(); }
...
///////////////////////////////////////////////////////////////////
void flash() {
  flashscreen = load_bitmap("bitmaps/flashscreen.bmp",
                            nullptr);  // flashscreen for ALL-GUI
  set_gfx_mode(GFX_AUTODETECT_WINDOWED, flashscreen->w + 100,
               flashscreen->h + 100, 0, 0);
  blit(flashscreen, screen, 0, 0,
       ceil((double)SCREEN_W / 2 - (double)flashscreen->w / 2.0),
       ceil((double)SCREEN_H / 2 - (double)flashscreen->h / 2.0),
       flashscreen->w, flashscreen->h);  /// this makes screen black too
  rest(1000);
  ...
  edgeOriginals[0] = load_bitmap("bitmaps/edge0001.bmp", nullptr);
  edgeOriginals[1] = load_bitmap("bitmaps/edge0005.bmp", nullptr);
  edgeOriginals[2] = load_bitmap("bitmaps/edge0010.bmp", nullptr);
  edgeOriginals[3] = load_bitmap("bitmaps/edge0050.bmp", nullptr);
  edgeOriginals[4] = load_bitmap("bitmaps/edge0100.bmp", nullptr);
  edgeOriginals[5] = load_bitmap("bitmaps/edge0200.bmp", nullptr);
  edgeOriginals[6] = load_bitmap("bitmaps/edge0300.bmp", nullptr);
  edgeOriginals[7] = load_bitmap("bitmaps/edge0400.bmp", nullptr);
  edgeOriginals[8] = load_bitmap("bitmaps/edge0500.bmp", nullptr);
  edgeOriginals[9] = load_bitmap("bitmaps/edge0600.bmp", nullptr);
  edgeOriginals[10] = load_bitmap("bitmaps/edge0700.bmp", nullptr);
  edgeOriginals[11] = load_bitmap("bitmaps/edge0800.bmp", nullptr);
  edgeOriginals[12] = load_bitmap("bitmaps/edge0900.bmp", nullptr);
  edgeOriginals[13] = load_bitmap("bitmaps/edge1000.bmp", nullptr);

  arrowOriginals[0] = load_bitmap("bitmaps/arrow0001.bmp", nullptr);
  arrowOriginals[1] = load_bitmap("bitmaps/arrow0005.bmp", nullptr);
  arrowOriginals[2] = load_bitmap("bitmaps/arrow0010.bmp", nullptr);
  arrowOriginals[3] = load_bitmap("bitmaps/arrow0050.bmp", nullptr);
  arrowOriginals[4] = load_bitmap("bitmaps/arrow0100.bmp", nullptr);
  arrowOriginals[5] = load_bitmap("bitmaps/arrow0200.bmp", nullptr);
  arrowOriginals[6] = load_bitmap("bitmaps/arrow0300.bmp", nullptr);
  arrowOriginals[7] = load_bitmap("bitmaps/arrow0400.bmp", nullptr);
  arrowOriginals[8] = load_bitmap("bitmaps/arrow0500.bmp", nullptr);
  arrowOriginals[9] = load_bitmap("bitmaps/arrow0600.bmp", nullptr);
  arrowOriginals[10] = load_bitmap("bitmaps/arrow0700.bmp", nullptr);
  arrowOriginals[11] = load_bitmap("bitmaps/arrow0800.bmp", nullptr);
  arrowOriginals[12] = load_bitmap("bitmaps/arrow0900.bmp", nullptr);
  arrowOriginals[13] = load_bitmap("bitmaps/arrow1000.bmp", nullptr);

  tailOriginals[0] = load_bitmap("bitmaps/tail0001.bmp", nullptr);
  tailOriginals[1] = load_bitmap("bitmaps/tail0005.bmp", nullptr);
  tailOriginals[2] = load_bitmap("bitmaps/tail0010.bmp", nullptr);
  tailOriginals[3] = load_bitmap("bitmaps/tail0050.bmp", nullptr);
  tailOriginals[4] = load_bitmap("bitmaps/tail0100.bmp", nullptr);
  tailOriginals[5] = load_bitmap("bitmaps/tail0200.bmp", nullptr);
  tailOriginals[6] = load_bitmap("bitmaps/tail0300.bmp", nullptr);
  tailOriginals[7] = load_bitmap("bitmaps/tail0400.bmp", nullptr);
  tailOriginals[8] = load_bitmap("bitmaps/tail0500.bmp", nullptr);
  tailOriginals[9] = load_bitmap("bitmaps/tail0600.bmp", nullptr);
  tailOriginals[10] = load_bitmap("bitmaps/tail0700.bmp", nullptr);
  tailOriginals[11] = load_bitmap("bitmaps/tail0800.bmp", nullptr);
  tailOriginals[12] = load_bitmap("bitmaps/tail0900.bmp", nullptr);
  tailOriginals[13] = load_bitmap("bitmaps/tail1000.bmp", nullptr);
...
void initialize_topbuttons() {
  int xplace = 0;
  topbutton[0] = load_bitmap("bitmaps/buttonlogo.bmp", nullptr);
  blit(topbutton[0], topbuttons, 0, 0, xplace, 0, topbutton[0]->w,
       topbutton[0]->h);
  xplace += topbutton[0]->w;

  topbutton[1] = load_bitmap("bitmaps/buttonspans.bmp", nullptr);
  blit(topbutton[1], topbuttons, 0, 0, xplace, 0, topbutton[1]->w,
       topbutton[1]->h);
  xplace += topbutton[1]->w;

  topbutton[2] = load_bitmap("bitmaps/buttonmax.bmp", nullptr);
  blit(topbutton[2], topbuttons, 0, 0, xplace, 0, topbutton[2]->w,
       topbutton[2]->h);
  xplace += topbutton[2]->w;

  topbutton[3] = load_bitmap("bitmaps/buttonavg.bmp", nullptr);
  blit(topbutton[3], topbuttons, 0, 0, xplace, 0, topbutton[3]->w,
       topbutton[3]->h);
  xplace += topbutton[3]->w;

  topbutton[4] = load_bitmap("bitmaps/buttonindexes.bmp", nullptr);
  blit(topbutton[4], topbuttons, 0, 0, xplace, 0, topbutton[4]->w,
       topbutton[4]->h);
  xplace += topbutton[4]->w;

  topbutton[5] = load_bitmap("bitmaps/buttonwks.bmp", nullptr);
  blit(topbutton[5], topbuttons, 0, 0, xplace, 0, topbutton[5]->w,
       topbutton[5]->h);
  xplace += topbutton[5]->w;

  topbutton[6] = load_bitmap("bitmaps/buttonfailsrc.bmp", nullptr);
  blit(topbutton[6], topbuttons, 0, 0, xplace, 0, topbutton[6]->w,
       topbutton[6]->h);
  xplace += topbutton[6]->w;

  topbutton[7] = load_bitmap("bitmaps/buttonfaildest.bmp", nullptr);
  blit(topbutton[7], topbuttons, 0, 0, xplace, 0, topbutton[7]->w,
       topbutton[7]->h);
  xplace += topbutton[7]->w;

  topbutton[8] = load_bitmap("bitmaps/buttonkey.bmp", nullptr);
  blit(topbutton[8], topbuttons, 0, 0, xplace, 0, topbutton[8]->w,
       topbutton[8]->h);
  xplace += topbutton[8]->w;

  topbutton[9] = load_bitmap("bitmaps/buttontopstretch.bmp", nullptr);
  stretch_blit(topbutton[9], topbuttons, 0, 0, topbutton[9]->w, topbutton[9]->h,
               xplace, 0, SCRNWID - xplace, topbutton[9]->h);

  topbuttonpress[0] = load_bitmap("bitmaps/buttonlogopress.bmp", nullptr);
  topbuttonpress[1] = load_bitmap("bitmaps/buttonspanspress.bmp", nullptr);
  topbuttonpress[2] = load_bitmap("bitmaps/buttonmaxpress.bmp", nullptr);
  topbuttonpress[3] = load_bitmap("bitmaps/buttonavgpress.bmp", nullptr);
  topbuttonpress[4] = load_bitmap("bitmaps/buttonindexespress.bmp", nullptr);
  topbuttonpress[5] = load_bitmap("bitmaps/buttonwkspress.bmp", nullptr);
  topbuttonpress[6] = load_bitmap("bitmaps/buttonfailsrcpress.bmp", nullptr);
  topbuttonpress[7] = load_bitmap("bitmaps/buttonfaildestpress.bmp", nullptr);
  topbuttonpress[8] = load_bitmap("bitmaps/buttonkeypress.bmp", nullptr);

  menubutton[0] = load_bitmap("bitmaps/menurun.bmp", nullptr);
  menubuttonover[0] = load_bitmap("bitmaps/menurunover.bmp", nullptr);
  menubutton[1] = load_bitmap("bitmaps/menuload.bmp", nullptr);
  menubuttonover[1] = load_bitmap("bitmaps/menuloadover.bmp", nullptr);
  menubutton[2] = load_bitmap("bitmaps/menubuild.bmp", nullptr);
  menubuttonover[2] = load_bitmap("bitmaps/menubuildover.bmp", nullptr);
  menubutton[3] = load_bitmap("bitmaps/menuedit.bmp", nullptr);
  menubuttonover[3] = load_bitmap("bitmaps/menueditover.bmp", nullptr);
  menubutton[4] = load_bitmap("bitmaps/menuexit.bmp", nullptr);
  menubuttonover[4] = load_bitmap("bitmaps/menuexitover.bmp", nullptr);

  topomenuover[0] = load_bitmap("bitmaps/topobuildnewpress.bmp", nullptr);
  topomenuover[1] = load_bitmap("bitmaps/topoeditexistingpress.bmp", nullptr);
}
...
void initialize_topobuttons() {
  int xplace = 0;
  topobutton[0] = load_bitmap("bitmaps/buttonlogo.bmp", nullptr);
  blit(topobutton[0], topbuttons, 0, 0, xplace, 0, topobutton[0]->w,
       topobutton[0]->h);
  xplace += topobutton[0]->w;

  topobutton[1] = load_bitmap("bitmaps/buttonplacerouter.bmp", nullptr);
  topobuttonover[0] = load_bitmap("bitmaps/buttonplacerouterover.bmp", nullptr);
  blit(topobutton[1], topbuttons, 0, 0, xplace, 0, topobutton[1]->w,
       topobutton[1]->h);
  xplace += topobutton[1]->w;

  topobutton[2] = load_bitmap("bitmaps/buttonplaceedge.bmp", nullptr);
  topobuttonover[1] = load_bitmap("bitmaps/buttonplaceedgeover.bmp", nullptr);
  blit(topobutton[2], topbuttons, 0, 0, xplace, 0, topobutton[2]->w,
       topobutton[2]->h);
  xplace += topobutton[2]->w;

  topobutton[3] = load_bitmap("bitmaps/buttonmoverouter.bmp", nullptr);
  topobuttonover[2] = load_bitmap("bitmaps/buttonmoverouterover.bmp", nullptr);
  blit(topobutton[3], topbuttons, 0, 0, xplace, 0, topobutton[3]->w,
       topobutton[3]->h);
  xplace += topobutton[3]->w;

  topobutton[4] = load_bitmap("bitmaps/buttoneditrouter.bmp", nullptr);
  topobuttonover[3] = load_bitmap("bitmaps/buttoneditrouterover.bmp", nullptr);
  blit(topobutton[4], topbuttons, 0, 0, xplace, 0, topobutton[4]->w,
       topobutton[4]->h);
  xplace += topobutton[4]->w;

  topobutton[5] = load_bitmap("bitmaps/buttoneditedge.bmp", nullptr);
  topobuttonover[4] = load_bitmap("bitmaps/buttoneditedgeover.bmp", nullptr);
  blit(topobutton[5], topbuttons, 0, 0, xplace, 0, topobutton[5]->w,
       topobutton[5]->h);
  xplace += topobutton[5]->w;

  topobutton[6] = load_bitmap("bitmaps/buttontopstretch.bmp", nullptr);
  stretch_blit(topobutton[6], topbuttons, 0, 0, topobutton[6]->w,
               topobutton[6]->h, xplace, 0, SCRNWID - xplace, topobutton[6]->h);
}
...
void initialize_graphbuttons() {
  int xplace = 0;
  graphbutton[0] = load_bitmap("bitmaps/buttonleft.bmp", nullptr);
  blit(graphbutton[0], graphbuttons, 0, 0, xplace, 0, graphbutton[0]->w,
       graphbutton[0]->h);
  xplace += graphbutton[0]->w;
  graphbutton[1] = load_bitmap("bitmaps/buttonright.bmp", nullptr);
  blit(graphbutton[1], graphbuttons, 0, 0, xplace, 0, graphbutton[1]->w,
       graphbutton[1]->h);
  xplace += graphbutton[1]->w;
  graphbutton[2] = load_bitmap("bitmaps/buttonpointer.bmp", nullptr);
  blit(graphbutton[2], graphbuttons, 0, 0, xplace, 0, graphbutton[2]->w,
       graphbutton[2]->h);
  xplace += graphbutton[2]->w;
  graphbutton[3] = load_bitmap("bitmaps/buttonzoomin.bmp", nullptr);
  blit(graphbutton[3], graphbuttons, 0, 0, xplace, 0, graphbutton[3]->w,
       graphbutton[3]->h);
  xplace += graphbutton[3]->w;
  graphbutton[4] = load_bitmap("bitmaps/buttonzoomout.bmp", nullptr);
  blit(graphbutton[4], graphbuttons, 0, 0, xplace, 0, graphbutton[4]->w,
       graphbutton[4]->h);
  xplace += graphbutton[4]->w;
  graphbutton[5] = load_bitmap("bitmaps/buttonzoomw.bmp", nullptr);
  blit(graphbutton[5], graphbuttons, 0, 0, xplace, 0, graphbutton[5]->w,
       graphbutton[5]->h);
  xplace += graphbutton[5]->w;
  graphbutton[6] = load_bitmap("bitmaps/buttonmove.bmp", nullptr);
  blit(graphbutton[6], graphbuttons, 0, 0, xplace, 0, graphbutton[6]->w,
       graphbutton[6]->h);
  xplace += graphbutton[6]->w;
  graphbutton[7] = load_bitmap("bitmaps/buttonstretch.bmp", nullptr);
  stretch_blit(graphbutton[7], graphbuttons, 0, 0, graphbutton[7]->w,
               graphbutton[7]->h, xplace, 0, SCRNWID - xplace,
               graphbutton[7]->h);

  graphbuttonpress[0] = load_bitmap("bitmaps/buttonleftpress.bmp", nullptr);
  graphbuttonpress[1] = load_bitmap("bitmaps/buttonrightpress.bmp", nullptr);
  graphbuttonpress[2] = load_bitmap("bitmaps/buttonpointerpress.bmp", nullptr);
  graphbuttonpress[3] = load_bitmap("bitmaps/buttonzoominpress.bmp", nullptr);
  graphbuttonpress[4] = load_bitmap("bitmaps/buttonzoomoutpress.bmp", nullptr);
  graphbuttonpress[5] = load_bitmap("bitmaps/buttonzoomwpress.bmp", nullptr);
  graphbuttonpress[6] = load_bitmap("bitmaps/buttonmovepress.bmp", nullptr);
}
...
void userBuildTopo(int argc, const char* argv[]) {
  ...
  numTopoEdges = 0;
  topofunction = 0;  // codes: 1 = addrouter, 2 = addedge, 3 = moverouter, 4 =
  ...
  while (true) {
    ...
    if (mouse_x > startX && mouse_x < startX + topomenu->w) {
      if (mouse_y > (startY + y1) && mouse_y < (startY + y2)) {
        blit(topomenuover[0], mainbuf, 0, 0, startX, startY + y1,
             topomenuover[0]->w, topomenuover[0]->h);
        if (mouse_b & 1) return true;
      } else if (mouse_y >= (startY + y2) && mouse_y < (startY + topomenu->h)) {
        blit(topomenuover[1], mainbuf, 0, 0, startX, startY + y2,
             topomenuover[1]->w, topomenuover[1]->h);
...
//					number to corresponding tool (given by
//tf).
//
///////////////////////////////////////////////////////////////////
void buildTopoSetFunction(int tf) {
  blit(topbuttons, mainbuf, 0, 0, 0, 0, topbuttons->w, topbuttons->h);
  blit(topobuttonover[tf - 1], mainbuf, 0, 0,
       topobutton[0]->w + topobutton[1]->w * (tf - 1), 0,
       topobuttonover[tf - 1]->w, topobuttonover[tf - 1]->h);
  masked_blit(mainbuf, screen, 0, 0, 0, 0, SCRNWID, SCRNHEI);
  if (mouse_b & 1)
    topofunction = tf;  // codes: 1 = addrouter, 2 = addedge, 3 = moverouter, 4
                        // = editrouter, 5 = editedge
}
...
void buildtopoplacerouter() {
  ...
  for (size_t r = 0; r < topoRouters.size(); ++r) {
    ...
    clickdist = sqrt((mosx - rx) * (mosx - rx) +
                     (mosy - ry) * (mosy - ry));  // distance formula
  ...
  if (freespot) {
    ...
    r->setIndex(sz);
    r->setXPercent(ceil(100.0 * ((double)mosx - (double)bkgrndX) /
                        ((double)realTopoWidthPx)));
    r->setYPercent(ceil(100.0 * ((double)mosy - (double)bkgrndY) /
                        ((double)realTopoHeightPx)));
    topoRouters.push_back(r);
    topoRouters[sz]->setXYPixels();
    topoRouters[sz]->moveXYPixels(
        0, topbuttons->h);  // initially translate below top button bar
...
void buildtopoplaceedge() {
  ...
  if (mosy < graphY1 && mosy > topbuttons->h) {
    ...
    if (to != -1 && to != from &&
        topoRouters[from]->getTopoEdgeByDest(to) == 0) {
      Edge* e1 = new Edge();
...
void buildtopomoverouter() {
  ...
  while (mouse_b & 1) {
    clear_to_color(edgesbmp, makecol(255, 0, 255));
    topoRouters[chosen]->moveXYPixels(
        diffx, diffy);  // remember, because we only change the pixels, we must
                        // recalculate x,yPercents before saving this topology
    for (unsigned int adj = 0; adj < topoRouters.size(); adj++) {
...
void buildtopoeditrouter() {
  int chosen = -1;  // TODO: These first fifteen or so lines of code are reused
                    // in about 3 such functions.
  int mosx = mouse_x;  // As one function, they would only need to return a
  ...
  if ((startX + 140) > SCRNWID) startX = topoRouters[chosen]->getXPixel() - 150;
  if ((startY + 100) > graphY1) startY = graphY1 - 100;
  if ((startY < 15)) startY = 15;
  masked_blit(editrouterinfo, popup, 0, 0, startX, startY, editrouterinfo->w,
              editrouterinfo->h);
  numtopaint = chosen;
  textprintf_ex(popup, font, startX + 65, startY + 10, black, -1, "%d",
                numtopaint);
  textprintf_ex(popup, font, startX + 15, startY + 35, black, -1, "\"%s\"",
                topoRouters[chosen]->getName());
  numtopaint = topoRouters[chosen]->getNumWorkstations();
  textprintf_right_ex(popup, font, startX + 129, startY + 57, black, -1, "%d",
                      numtopaint);
  numtopaint = 100.0 * double(topoRouters[chosen]->getNumWorkstations()) /
               double(numTopoWorkstations);
  ...
  if (mouse_x > startX && mouse_x < startX + editrouterinfo->w &&
      mouse_y > startY && mouse_y < startY + editrouterinfo->h) {
    if (mouse_y > startY + 25 && mouse_y < startY + 47) {
      topoRouters[chosen]->setName(edit_inputter(false, startX, startY + 25,
                                                 editrouterinfo->w, 47 - 25));
    } else if (mouse_y > startY + 47 && mouse_y < startY + 68) {
      int numw = topoRouters[chosen]->getNumWorkstations();
      topoRouters[chosen]->setNumWorkstations(edit_inputint(
          false, topoRouters[chosen]->getNumWorkstations(), startX + 90,
          startY + 47, editrouterinfo->w - 94, 68 - 47));
...
void buildtopoeditedge() {
  int from = -1;  // this is copied too. Again, TODO: we should replace with a
  ...
  if (mosy < graphY1 && mosy > topbuttons->h) {
    ...
    if (edge != 0) {
      clear_to_color(popup, makecol(255, 0, 255));
      int startX =
          ceil(double(topoRouters[edge->getSourceIndex()]->getXPixel() +
                      topoRouters[edge->getDestinationIndex()]->getXPixel()) /
                   2.0 -
               double(editedgeinfo->w) / 2.0);
      int startY =
          ceil(double(topoRouters[edge->getSourceIndex()]->getYPixel() +
                      topoRouters[edge->getDestinationIndex()]->getYPixel()) /
                   2.0 -
               double(editedgeinfo->h) / 2.0);
      int numtopaint;
      int black = makecol(0, 0, 0);
      ...
      if ((startY + 50) > graphY1) startY = graphY1 - 50;
      if ((startY < 15)) startY = 15;

      masked_blit(editedgeinfo, popup, 0, 0, startX, startY, editedgeinfo->w,
                  editedgeinfo->h);
      numtopaint = edge->getSourceIndex();
      textprintf_centre_ex(popup, font, startX + 50, startY + 10, black, -1,
                           "%d", numtopaint);
      numtopaint = edge->getDestinationIndex();
      textprintf_centre_ex(popup, font, startX + 85, startY + 10, black, -1,
                           "%d", numtopaint);
      numtopaint = edge->getNumberOfSpans();
      textprintf_right_ex(popup, font, startX + editedgeinfo->w - 7,
                          startY + 32, black, -1, "%3d", numtopaint);
      topoRefresh();
      // COPY COPY COPY COPY COPY COPY COPY COPY COPY COPY COPY
      // paint editrouterinfo to mainbuf,then to screen
      while (mouse_b & 1) {
      ...
      if (mouse_x > startX && mouse_x < startX + editedgeinfo->w &&
          mouse_y > startY && mouse_y < startY + editedgeinfo->h) {
        if (mouse_y > startY + 25 && mouse_y < startY + 46) {
          int num_spans =
              edit_inputint(false, edge->getNumberOfSpans(), startX + 60,
                            startY + 25, editedgeinfo->w - 64, 46 - 25);
          edge->setNumberOfSpans(num_spans);
          edge = edge = topoRouters[to]->getTopoEdgeByDest(from);
          edge->setNumberOfSpans(num_spans);
        } else if (mouse_y > startY + 47 &&
                   mouse_y < startY + editedgeinfo->h) {
...
//					pixel ratio represented by input
//parameters
//
///////////////////////////////////////////////////////////////////
...
const char* edit_inputter(bool alignleft, int xpx, int ypx, int wid, int hei) {
  ...
  while (!key[KEY_ENTER] &&
         !(mouse_b & 1)) {  // loop puts name of directory together.
    if (keypressed()) {
      rectfill(screen, xpx, ypx, xpx + wid, ypx + hei, makecol(175, 175, 175));
      ...
      if (place < 19) {
        ...
        place++;
        if (alignleft)
          textprintf_right_ex(screen, font, xpx + 50, ypx + 5, black, -1, "%s",
                              getter);
        else
          textprintf_right_ex(screen, font, xpx + wid - 10, ypx + 5, black, -1,
                              "%s", getter);
        ...
        if (place > 0) {
          ...
          if (alignleft)
            textprintf_right_ex(screen, font, xpx + 50, ypx + 5, black, -1,
                                "%s", getter);
          else
            textprintf_right_ex(screen, font, xpx + wid - 10, ypx + 5, black,
                                -1, "%s", getter);
...
int edit_inputint(bool alignleft, int edit, int xpx, int ypx, int wid,
                  int hei) {
  int num = edit;
  int black = makecol(0, 0, 0);
  rectfill(screen, xpx, ypx, xpx + wid, ypx + hei, makecol(175, 175, 175));
  ...
  if (alignleft)
    ...
    textprintf_right_ex(screen, font, xpx + wid - 10, ypx + 7, black, -1, "%d",
                        num);
  while (mouse_b & 1) {
  }
  while (
      !key[KEY_ENTER] &&
      !(mouse_b &
        1)) {  // loop lets user push up/down arrows to increase value of edit
    if (key[KEY_UP] || key[KEY_RIGHT]) {
      ...
      if (alignleft)
        ...
        textprintf_right_ex(screen, font, xpx + wid - 10, ypx + 7, black, -1,
                            "%d", num);
...
void userLoadPrevSim(int argc, const char* argv[]) {
  ...
  bool folderselected =
      false;       // says whether directory has been successfully appended
  int levels = 0;  // currently set to go down two levels: index.idx ->
                   // folder/index.idx -> folder/selection.thd
  int black = makecol(0, 0, 0);    // define color: black
  int green = makecol(0, 255, 0);  // define color: green
  white = makecol(255, 255, 255);

  string indexName = "index.idx";  // string used to name whatever folder/file
                                   // we currently want
  string indexName2;
  string directory;  // string used to hold names of directories taken from
                     // input file.
  char directory1[25];  // used to append second directory to first
  char buffer[50];
  char taker[30];
  string taker2;
  vector<string> directories;  // vector to hold names of things in currently
                               // viewed index.idx file
  int result;  // used to keep track of the length of directories
  int mb, mx, my, spcng = 15,
                  printHdSt = 100;  // mouse variables; spacing, printHdSt, and
                                    // printHd for painting list
  int printHd;
  ...
  while (levels < 2) {
    ...
    rectfill(screen, SCRNWID - 50, SCRNHEI - 50, SCRNWID, SCRNHEI,
             makecol(255, 0, 0));
    printHd = printHdSt;
    result = 0;
    ...
    if (levels == 0) {
      ...
      while (inputFile.getline(
          buffer, 40))  // Before bed... having trouble understanding why it
                        // can't read this file the second time around.
      {
        directory = strtok(buffer, "|");
        directories.push_back(directory);
        textprintf_ex(screen, font, 100, printHd, black, -1, "%s",
                      directories.at(result).c_str());
    ...
    while (!folderselected) {
      ...
      mx = mouse_x;
      if (my > printHdSt &&
          my < printHd)  // if on one of the directory options.
      {
        result = (my - printHdSt) / spcng;
        for (unsigned int p = 0; p < directories.size(); p++) {
          if (p != result)
            textprintf_ex(screen, font, 100, p * spcng + printHdSt, black,
                          makecol(214, 214, 214), "%s",
                          directories.at(p).c_str());
          else
            textprintf_ex(screen, font, 100, result * spcng + printHdSt, black,
                          makecol(175, 175, 175), "%s",
                          directories.at(p).c_str());
        }
        if (mb) {  // mouse clicked, see if directory is legit.
          ...
          if (fileCheck == nullptr) {
            textprintf_ex(screen, font, 100, result * spcng + printHdSt, black,
                          green, "Directory Not Found");
            directories.erase(directories.begin() + result);
            printHd -= spcng;
            // clear_to_color(screen,makecol(214,214,214));
            rectfill(screen, 75, 50, 400, SCRNHEI - 100,
                     makecol(214, 214, 214));
            outputFile.open(indexName.c_str(), std::ios::out);
            for (unsigned int d = 0; d < directories.size(); d++) {
              textprintf_ex(screen, font, 100, d * spcng + printHdSt, black, -1,
                            "%s", directories.at(d).c_str());
              outputFile << directories.at(d).c_str() << "|\n";
            }
            outputFile.close();
          } else  // directory is cool
          {
            // textprintf_ex(screen,font,100,result*spcng +
            // printHdSt,white,black,"Found");
            rest(200);
            fclose(fileCheck);
            if (levels ==
                0)  // sets it up to open the right index file next time around
  ...
  inputFile3.getline(taker, 30);
  taker2 = strtok(taker, "|");
  threadZero->setAvgProbesPerRequest(
      atof(taker2.c_str()));  // avg probes per request
  inputFile3.getline(taker, 30);
  taker2 = strtok(taker, "|");
  threadZero->setAvgRequestDelayTime(
      atof(taker2.c_str()));  // setAvgRequestDelayTime
  inputFile3.getline(taker, 30);
  taker2 = strtok(taker, "|");
  ...
  inputFile3.getline(taker, 30);
  taker2 = strtok(taker, "|");
  threadZero->setNumberOfWavelengths(
      atoi(taker2.c_str()));  // available wavelengths
  inputFile3.getline(taker, 30);
  taker2 = strtok(taker, "|");
...
void menu() {
  ...
  while (!esc) {
    ...
    if (mx < 100 && my > topbuttons->h &&
        my < topbuttons->h + 5 * menubutton[0]->h) {
      if (mb) esc = true;
      if (my <
          topbuttons->h +
              menubutton[0]
                  ->h)  // blit(menu,mainbuf,0,0,0,topbuttons->h,menu->w,menu->h);
      {
        blit(menubuttonover[0], menu, 0, 0, 0, 0, menubutton[0]->w,
             menubutton[0]->h);
        if (mb) {
          functionactive = 1;
        }
      } else if (my < topbuttons->h + 2 * menubutton[0]->h) {
        blit(menubuttonover[1], menu, 0, 0, 0, menubutton[1]->h,
             menubutton[1]->w, menubutton[1]->h);
        if (mb) {
          functionactive = 2;
        }
      } else if (my < topbuttons->h + 3 * menubutton[0]->h) {
        blit(menubuttonover[2], menu, 0, 0, 0, 2 * menubutton[2]->h,
             menubutton[2]->w, menubutton[2]->h);
        if (mb) {
          functionactive = 3;
        }
      } else if (my < topbuttons->h + 4 * menubutton[0]->h) {
        blit(menubuttonover[3], menu, 0, 0, 0, 3 * menubutton[3]->h,
             menubutton[3]->w, menubutton[3]->h);
        if (mb) {
          functionactive = 4;
        }
      } else if (my < topbuttons->h + 5 * menubutton[0]->h) {
        blit(menubuttonover[4], menu, 0, 0, 0, 4 * menubutton[4]->h,
             menubutton[4]->w, menubutton[4]->h);
        if (mb) {
          functionactive = 0;
        }
      }
...
void userRunNewSim(int argc, const char* argv[]) {
  ...
  if (fldcreate != 0) {
    textprintf_ex(screen, font, 20, 35, color2, color,
                  "Impossible. Returning to Menu.");
    rest(3000);
    return;
  } else {
