...
Edge::Edge(size_t src, size_t dest, size_t spans)
  ...
  for (int x = 0; x < 14; x++) edgeBmps[x] = 0;
...
void Edge::resetPheremone(size_t ci, size_t spans) {
  if (threads[ci]->getCurrentRoutingAlgorithm() == MAX_MIN_ACO) {
    ...
    pheremone = double(threadZero->getQualityParams().ACO_ants) / double(spans);
  }
}
...
// Description:		Adds the pheremone level based upon the solution
// quality.
//
///////////////////////////////////////////////////////////////////
...
void Edge::scaleEdgesTo(int spns, int px) {
  double dist, doubR1X, doubR1Y, doubR2X, doubR2Y;
  doubR1X = (double)topoRouters[sourceIndex]
                ->getXPixel();  // set coordinates of routers
  ...
  dist = sqrt((doubR2X - doubR1X) * (doubR2X - doubR1X) +
              (doubR2Y - doubR1Y) * (doubR2Y - doubR1Y));  // distance formula
...
void Edge::removeEstablishedConnection(void *dcpe_void) {
  DestroyConnectionProbeEvent *dcpe =
      static_cast<DestroyConnectionProbeEvent *>(dcpe_void);
  ...
  if (ec && threadZero->getQualityParams().q_factor_stats == true) {
    ...
    for (s = 0; s < ec->QFactors->size() - 1; ++s) {
      weight +=
          ec->QFactors->at(s) * (ec->QTimes->at(s + 1) - ec->QTimes->at(s));
    ...
    weight += ec->QFactors->at(s) * (ec->connectionEndTime - ec->QTimes->at(s));
    ...
    ec->averageQFactor =
        double(weight / (ec->connectionEndTime - ec->connectionStartTime));

    ec->belowQFactor =
        double(timebelow / (ec->connectionEndTime - ec->connectionStartTime));

#ifndef NO_ALLEGRO
    int src = dcpe->connectionPath[0]->getSourceIndex();
    threads[thdIndx]->getRouterAt(src)->addToAvgQFrom(ec->averageQFactor);
    int dest =
        dcpe->connectionPath[dcpe->connectionLength - 1]->getDestinationIndex();
...
void Edge::updateGUI() {
  ...
  usageList.push_back(
      actualUsage);  // put newUsage number into usage history vector
...
void Edge::calculateAverageUsage() {
  ...
  for (unsigned int u = threadZero->getUsageHistStartTime();
       u < (threadZero->getUsageHistStartTime() +
            threadZero->getNumUsagesToDisplay());
       ++u) {
    total += usageList[u];
  }
...
void Edge::paintSpans() {
  ...
  textprintf_centre_ex(edgespans, font, x, y, makecol(255, 255, 255),
                       makecol(0, 0, 0), "%d", numberOfSpans);
}
...
void Edge::paintUsage(int p) {
  ...
  if (percent != painted_percent ||
      p == -5)  // p == -4,-3,or-2 will be sign that we are painting over again.
  {
    al_masked_blit(edgeBmps[percent], edgesbmp, 0, 0, putX - 7, putY - 7,
                   edgeBmps[0]->w, edgeBmps[0]->h);
...
void Edge::paintUsageHistory(int x1, int y1, int x2, int y2, int t) {
  ...
  if (t > 0) {
    ...
    ai_clear_to_color(
        graph,
        makecol(255, 0, 255));  // clear graph because we're making a new one.
    ...
    stretch_blit(graphbackground, graph, 0, 0, graphbackground->w,
                 graphbackground->h, 0, y1, x2 - x1, y2 - y1);
    realBH = y2 - 20.0;
    int drawnUnitPx = y2;
    ...
    while (bheight > y1 + 10)  // add ten px just to make it cleaner
    {
      if (bheight <
          (drawnUnitPx -
           15.0)) {  // 10 px used as safe distance to put between numbers
        rectfill(graph, x - 5, bheight + 1, x, bheight,
                 col);  // unit mark (10 px by 1 px)
        textprintf_right_ex(graph, font, x - 6, bheight - 5, col, pink, "%d",
                            unit);
        drawnUnitPx = bheight;
      }
    ...
    textprintf_right_ex(graph, font, x2 - 5, y1 - 10, bla, pink,
                        "From %d to %d", sourceIndex, destinationIndex);
    ...
    for (unsigned int u = threadZero->getUsageHistStartTime();
      ...
      use = usageList[u];
      if (use <= mid) {  // we'll say 2 is our yellow point for now. TODO: make
                         // constant
        green = 255;
        red = 255 * (use / mid);
      } else {
      ...
      if (u % timeUnitModder == 0)  // if true, draw unit marker
      {
        ...
        textprintf_centre_ex(graph, font, x, y2 - 10, truegreen, bla, "%d",
                             u * timeFact);
      }
    ...
    bheight = realBH;
    rectfill(graph, x, y2 - bheight, x2 - 10, y2 - bheight,
             col);  // average line
    textprintf_right_ex(graph, font, x2 - 10, y2 - (bheight + 10), col, bla,
                        "average = %f", average_usage);
    textprintf_right_ex(graph, font, x2 - 20, y1 + 25, truegreen, bla,
                        "Maximum Usage: %d", max_actual_usage);
  }
}
