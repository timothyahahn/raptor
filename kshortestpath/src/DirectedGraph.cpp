...
DirectedGraph::DirectedGraph(kShortestPathParms params)
  ...
  for (size_t a = 0; a < params.total_edges; ++a) {
    ...
    m_pDirectedEdges->insert(std::pair<std::pair<size_t, size_t>, double>(
        std::pair<size_t, size_t>(i, j), w));
...
DirectedGraph& DirectedGraph::operator=(const DirectedGraph& rGraph) {
  ...
  m_pDirectedEdges =
      new ConfigCenter::SizeT_Pair_Double_Map(*(rGraph.m_pDirectedEdges));
...
void DirectedGraph::RemoveEdge(size_t i, size_t j) {
  ConfigCenter::SizeT_Pair_Double_Map_Iterator pos =
      m_pDirectedEdges->find(ConfigCenter::SizeT_Pair(i, j));
